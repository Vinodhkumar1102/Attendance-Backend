...
const sendResetEmail = async (email, resetCode) => {
  ...
  console.log('Admin reset code email sent successfully', {
    email,
    emailId: data?.id,
  });
...
const sendPushNotificationToAdmin = async (adminEmail, notification) => {
  try {
    ...
    console.log('Admin push sent successfully', {
      messageId,
      adminEmail,
    });
    ...
    return {
      sent: false,
      reason:
        error.message || 'Unable to send admin push notification',
    };
  }
};
...
const adminLogin = async (request, response) => {
  try {
    ...
    if (
      ...
      return response
        .status(500)
        .json({message: 'Admin login configuration is missing'});
    }
...
const adminLogout = (request, response) => {
  ...
  if (!token) {
    return response
      .status(400)
      .json({message: 'Bearer token is required'});
  }

  revokeToken(token);

  return response.json({
    message: 'Admin logout successful. Remove the token from the client.',
  });
};
...
const saveAdminDeviceToken = async (request, response) => {
  try {
    const deviceToken = getValue(
      request.body.deviceToken || request.body.token,
    );

    if (!deviceToken) {
      return response
        .status(400)
        .json({message: 'Device token is required'});
    }
    ...
    return response.json({
      message: 'Admin device token registered',
      deviceToken: adminUser.deviceToken,
    });
  } catch (error) {
    console.error('Save admin device token error:', error);

    return response
      .status(500)
      .json({message: 'Unable to save admin device token'});
  }
};
...
const getAdminNotifications = async (request, response) => {
  try {
    const adminRecord = await Admin.findById(
      ...
      notifications.map(async notification => {
        ...
        return avatarPath
          ? {
              ...
              avatarUrl: `${avatarBaseUrl}${avatarPath}`,
            }
          : notification;
      }),
    );
    ...
    return response
      .status(500)
      .json({message: 'Unable to fetch admin notifications'});
  }
};
...
const sendAdminResetCode = async (request, response) => {
  try {
    ...
    if (!admin) {
      return response
        .status(404)
        .json({message: 'Admin account was not found'});
    }
    ...
    return response.status(503).json({
      message:
        'Email service is unavailable. Please check the Resend configuration.',
    });
  }
};
...
const resetAdminPassword = async (request, response) => {
  try {
    ...
    const admin = await Admin.findOne({email}).select(
      '+password +resetCode +resetCodeExpiresAt',
    );
